...
export default function FootballHome() {
  ...
  const loadSchedule = async () => {
    ...
    if (data) {
      ...
      // Debug logging
      console.log('Football Schedule Load:', {
  ...
  useEffect(() => {
    if (exercise.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'football', query: exercise.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
        ...
        if (exerciseType) {
          const metrics = getMetricOptionsForExerciseType(exerciseType, 'football');
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerTopRow}>
          ...
          <Text style={styles.headerBrand}>Football</Text>
          ...
          <View style={styles.headerRight}>
            ...
            >
              <Ionicons name="american-football-outline" size={22} color="#FFFFFF" />
            </Pressable>
          </View>
        </View>
            ...
            onPress={() => {
              ...
              if (gameCardRef.current) {
                gameCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/football/add-game",
                    params: {
                      imageSource: "allen",
                ...
                router.push("/(tabs)/(home)/football/add-game");
              }
            }}
          onPressIn={() => {
            ...
            onPress={() => {
              ...
              if (practiceCardRef.current) {
                practiceCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/football/add-practice",
                    params: {
                      imageSource: "jefferson",
                      initialX: String(pageX),
                      initialY: String(pageY),
                      initialWidth: String(width),
                      initialHeight: String(height),
                ...
                router.push("/(tabs)/(home)/football/add-practice");
              }
            }}
          onPressIn={() => {
