// app/(tabs)/(home)/baseball/index.tsx
...
function IntervalChip({
  ...
  // (kept for parity with other screens; not used by the graph)
  return (
    <Pressable
      onPress={onPress}
      style={[
...
export default function BaseballHome() {
  ...
  const loadGoals = async () => {
    ...
    const { data, error } = await listWeeklyGoals({ mode: 'baseball' });
  ...
  const [availableMetrics, setAvailableMetrics] = useState<string[]>(["reps", "distance"]);
  ...
  useEffect(() => {
    if (query.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'baseball', query: query.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
        ...
        if (exerciseType) {
          ...
          const metrics = getMetricOptionsForExerciseType(exerciseType, 'baseball');
      ...
      // Default metrics for baseball when no query
      setCurrentExerciseType(null);
      setAvailableMetrics(["reps", "distance"]);
      setMetric("reps");
    }
  }, [query, user]);
  ...
  return (
    ...
    >
      ...
      <Pressable
        ...
        onPress={() => router.push("/(tabs)/(home)/baseball/weekly-goals")}
      >
        <LinearGradient
            ...
            >
              <Text style={styles.dropdownText}>
                {metric === "reps" ? "Reps" : 
                 ...
                 metric === "distance" ? (currentExerciseType === "hitting" ? "Avg. Distance" : "Distance") :
                  ...
                  >
                    <Text style={styles.menuText}>
                      {k === "reps" ? "Reps" : 
                       ...
                       k === "distance" ? (currentExerciseType === "hitting" ? "Avg. Distance" : "Distance") :
                       k === "time_min" ? "Time (min)" :
      ...
      <View style={{ gap: 20, marginTop: 12 }}>
        <PremiumGatedCard
          image={require("../../../../assets/players/ohtani.jpg")}
          title="Log Game"
          subtitle="For Optimized Training"
          buttonText="+ Add Game"
          onPress={() => router.push("/(tabs)/(home)/baseball/add-game")}
          isPremium={canLogGames}
          featureName="Log Game"
        />

        <PremiumGatedCard
          image={require("../../../../assets/players/judge.webp")}
          title="Log Team Practices"
          subtitle="To Add Additional Training"
          buttonText="+ Add Practice"
          onPress={() => router.push("/(tabs)/(home)/baseball/add-practice")}
...
const styles = StyleSheet.create({
  // ✅ Only change: header now uses Geist (Font 2)
  ...
  /* kept for parity because IntervalChip is defined */
  chip: {
    borderRadius: 999,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: "rgba(255,255,255,0.18)",
