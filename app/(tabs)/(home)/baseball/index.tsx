...
export default function BaseballHome() {
  ...
  const loadSchedule = async () => {
    ...
    if (data) {
      ...
      // Debug logging
      console.log('Baseball Schedule Load:', {
  ...
  useEffect(() => {
    if (exercise.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'baseball', query: exercise.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerTopRow}>
          ...
          <Text style={styles.headerBrand}>Baseball</Text>
          ...
          <View style={styles.headerRight}>
            ...
            >
              <MaterialCommunityIcons name="baseball" size={22} color="#FFFFFF" />
            </Pressable>
          </View>
        </View>
      ...
      >
            ...
            onPress={() => {
              ...
              if (gameCardRef.current) {
                gameCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/baseball/add-game",
                    params: {
                      imageSource: "ohtani",
                ...
                router.push("/(tabs)/(home)/baseball/add-game");
              }
            }}
          onPressIn={() => {
        ...
        >
          <Animated.View style={[styles.logSectionInner, gameAnimatedStyle]}>
            <Image 
              source={require("../../../../assets/players/ohtani.jpg")} 
            ...
            onPress={() => {
              ...
              if (practiceCardRef.current) {
                practiceCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/baseball/add-practice",
                    params: {
                      imageSource: "judge",
                      initialX: String(pageX),
                      initialY: String(pageY),
                ...
                router.push("/(tabs)/(home)/baseball/add-practice");
              }
            }}
          onPressIn={() => {
        ...
        >
          <Animated.View style={[styles.logSectionInner, practiceAnimatedStyle]}>
            <Image 
              source={require("../../../../assets/players/judge.webp")} 
              style={styles.logImage}
              resizeMode="cover"
            />
            {/* Gradient Scrim Overlay */}
