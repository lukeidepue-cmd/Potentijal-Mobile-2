...
export default function TennisHome() {
  ...
  const loadSchedule = async () => {
    ...
    if (data) {
      ...
      // Debug logging
      console.log('Tennis Schedule Load:', {
  ...
  useEffect(() => {
    if (exercise.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'tennis', query: exercise.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerTopRow}>
          ...
          <Text style={styles.headerBrand}>Tennis</Text>
          ...
          <View style={styles.headerRight}>
            ...
            >
              <Ionicons name="tennisball-outline" size={22} color="#FFFFFF" />
            </Pressable>
          </View>
        </View>
      ...
      >
            ...
            onPress={() => {
              ...
              if (gameCardRef.current) {
                gameCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/tennis/add-game",
                    params: {
                      imageSource: "novak",
                ...
                router.push("/(tabs)/(home)/tennis/add-game");
              }
            }}
          onPressIn={() => {
        ...
        >
          <Animated.View style={[styles.logSectionInner, gameAnimatedStyle]}>
            <Image 
              source={require("../../../../assets/players/novak.webp")} 
            ...
            onPress={() => {
              ...
              if (practiceCardRef.current) {
                practiceCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/tennis/add-practice",
                    params: {
                      imageSource: "carlos",
                      initialX: String(pageX),
                      initialY: String(pageY),
                ...
                router.push("/(tabs)/(home)/tennis/add-practice");
              }
            }}
          onPressIn={() => {
        ...
        >
          <Animated.View style={[styles.logSectionInner, practiceAnimatedStyle]}>
            <Image 
              source={require("../../../../assets/players/carlos.webp")} 
              style={styles.logImage}
              resizeMode="cover"
            />
            {/* Gradient Scrim Overlay */}
