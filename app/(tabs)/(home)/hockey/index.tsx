...
export default function HockeyHome() {
  ...
  const loadGoals = async () => {
    ...
    const { data, error } = await listWeeklyGoals({ mode: 'hockey' });
  ...
  useEffect(() => {
    if (query.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'hockey', query: query.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
        ...
        if (exerciseType) {
          ...
          const metrics = getMetricOptionsForExerciseType(exerciseType, 'hockey');
      ...
      // Default metrics for hockey when no query
      setCurrentExerciseType(null);
  ...
  return (
    ...
    >
      ...
      {goals.length > 1 && (
        <View style={{ alignItems: "center", marginTop: 6, marginBottom: 6 }}>
          <View style={styles.dotsRow}>
            {goals.map((_, i) => <View key={`dot-${i}`} style={[styles.dot, i === page && styles.dotActive]} />)}
          </View>
        </View>
      )}
      ...
      <Pressable
        ...
        onPress={() => router.push("/(tabs)/(home)/hockey/weekly-goals")}
      >
      ...
      <View style={{ gap: 20, marginTop: 12 }}>
        <PremiumGatedCard
          image={require("../../../../assets/players/matthews.jpg")}
          title="Log Game"
          subtitle="For Optimized Training"
          buttonText="+ Add Game"
          onPress={() => router.push("/(tabs)/(home)/hockey/add-game")}
          isPremium={canLogGames}
          featureName="Log Game"
        />

        <PremiumGatedCard
          image={require("../../../../assets/players/mcdavid.jpg")}
          title="Log Team Practices"
          subtitle="To Add Additional Training"
          buttonText="+ Add Practice"
          onPress={() => router.push("/(tabs)/(home)/hockey/add-practice")}
