...
export default function HockeyHome() {
  ...
  const loadSchedule = async () => {
    ...
    if (data) {
      ...
      // Debug logging
      console.log('Hockey Schedule Load:', {
  ...
  useEffect(() => {
    if (exercise.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'hockey', query: exercise.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerTopRow}>
          ...
          <Text style={styles.headerBrand}>Hockey</Text>
          ...
          <View style={styles.headerRight}>
            ...
            >
              <MaterialCommunityIcons name="hockey-sticks" size={22} color="#FFFFFF" />
            </Pressable>
          </View>
        </View>
      ...
      >
            ...
            onPress={() => {
              ...
              if (gameCardRef.current) {
                gameCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/hockey/add-game",
                    params: {
                      imageSource: "mcdavid",
                ...
                router.push("/(tabs)/(home)/hockey/add-game");
              }
            }}
          onPressIn={() => {
        ...
        >
          <Animated.View style={[styles.logSectionInner, gameAnimatedStyle]}>
            <Image 
              source={require("../../../../assets/players/mcdavid.jpg")} 
            ...
            onPress={() => {
              ...
              if (practiceCardRef.current) {
                practiceCardRef.current.measure((x, y, width, height, pageX, pageY) => {
                  router.push({
                    pathname: "/(tabs)/(home)/hockey/add-practice",
                    params: {
                      imageSource: "matthews",
                      initialX: String(pageX),
                      initialY: String(pageY),
                      initialWidth: String(width),
                      initialHeight: String(height),
                ...
                router.push("/(tabs)/(home)/hockey/add-practice");
              }
            }}
          onPressIn={() => {
        ...
        >
          <Animated.View style={[styles.logSectionInner, practiceAnimatedStyle]}>
            <Image 
              source={require("../../../../assets/players/matthews.jpg")} 
              style={styles.logImage}
              resizeMode="cover"
            />
            {/* Gradient Scrim Overlay */}
