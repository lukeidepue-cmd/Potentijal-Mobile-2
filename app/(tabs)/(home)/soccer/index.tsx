// app/(tabs)/(home)/soccer/index.tsx
import React, { useMemo, useRef, useState, useEffect } from "react";
import {
  View, Text, StyleSheet, ScrollView, Pressable, TextInput,
  Dimensions, NativeScrollEvent, NativeSyntheticEvent, Image, ActivityIndicator, Platform
} from "react-native";
...
function pct(n: number, d: number) { return d ? Math.max(0, Math.min(1, n / d)) : 0; }

function IntervalChip({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={[styles.chip, active && { borderColor: theme.colors.primary600, backgroundColor: "#0E1316" }]}>
...
export default function SoccerHome() {
  ...
  const loadGoals = async () => {
    ...
    const { data, error } = await listWeeklyGoals({ mode: 'soccer' });
  ...
  useEffect(() => {
    if (query.trim() && user) {
      getPrimaryExerciseTypeDirect({ mode: 'soccer', query: query.trim() }).then(({ data: exerciseType, error }) => {
        if (error) {
        ...
        if (exerciseType) {
          const metrics = getMetricOptionsForExerciseType(exerciseType, 'soccer');
      ...
      // Default metrics for soccer when no query
  ...
  if (!fontsReady) return <View style={{ flex: 1, backgroundColor: theme.colors.bg0, alignItems: "center", justifyContent: "center" }}><ActivityIndicator/></View>;

  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.colors.bg0 }} contentContainerStyle={{ padding: theme.layout.xl, gap: theme.layout.lg, paddingBottom: 20 }} keyboardShouldPersistTaps="handled">
      <View style={{ alignItems: "center", marginTop: 34 }}>
        <Text style={styles.header}>Home</Text>
        <View style={styles.headerUnderline} />
      </View>

      <LinearGradient colors={["#0B0E10", "#10161B"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.sectionWrap}>
        <Text style={styles.kicker}>Weekly</Text>
        <Text style={styles.sectionTitle}>Goals</Text>

        <ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} snapToInterval={SNAP} decelerationRate="fast" onMomentumScrollEnd={onMomentumEnd} contentContainerStyle={{ paddingHorizontal: EDGE_PAD }}>
          {goalsLoading ? (
            ...
            goals.map((g) => {
              ...
              return (
                <View key={g.id} style={[styles.goalCard, { width: CARD_W, marginRight: CARD_GAP }]}>
                  ...
                  <View style={styles.trackBox}><View style={[styles.fill, { width: `${p * 100}%` }]} /></View>
                </View>
              );
            })
          )}
      ...
      {goals.length > 1 && (
        <View style={{ alignItems: "center", marginTop: 6, marginBottom: 6 }}>
          <View style={styles.dotsRow}>{goals.map((_, i) => <View key={`dot-${i}`} style={[styles.dot, i === page && styles.dotActive]} />)}</View>
        </View>
      )}

      <Pressable style={{ alignSelf: "flex-end", marginTop: 8, marginBottom: 24 }} onPress={() => router.push("/(tabs)/(home)/soccer/weekly-goals")}>
        <LinearGradient colors={[theme.colors.primary600, "#3BAA6F"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.addBtnGrad}>
      ...
      <View style={{ gap: 20, marginTop: 12 }}>
        <PremiumGatedCard
          image={require("../../../../assets/players/yamal.jpg")}
          title="Log Game"
          subtitle="For Optimized Training"
          buttonText="+ Add Game"
          onPress={() => router.push("/(tabs)/(home)/soccer/add-game")}
          isPremium={canLogGames}
          featureName="Log Game"
        />

        <PremiumGatedCard
          image={require("../../../../assets/players/mbappe.webp")}
          title="Log Team Practices"
          subtitle="To Add Additional Training"
          buttonText="+ Add Practice"
          onPress={() => router.push("/(tabs)/(home)/soccer/add-practice")}
...
const styles = StyleSheet.create({
  ...
  kicker: { color: theme.colors.textLo, letterSpacing: 0.6, textTransform: "uppercase", fontSize: 12, fontFamily: FONT.displayBold, marginBottom: 2 },
  sectionTitle: { color: "#FFFFFF", fontSize: 26, letterSpacing: 0.2, fontWeight: Platform.select({ ios: "900", android: "700" }) as any, marginBottom: 10 },

  goalCard: { backgroundColor: "rgba(12,16,20,0.85)", borderRadius: 16, borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,255,255,0.08)", padding: 14 },
  leftBar: { width: 4, height: 18, borderRadius: 4, backgroundColor: theme.colors.primary600, marginRight: 10 },
  goalName: { color: "#EAF3EE", fontSize: 18, fontFamily: FONT.uiBold, maxWidth: "60%" },
  goalSubtle: { color: "#BFC8C3", fontSize: 12, fontFamily: FONT.uiMedium },

  trackBox: { marginTop: 10, height: 14, borderRadius: 10, backgroundColor: "#0F1418", borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,255,255,0.08)", overflow: "hidden" },
  ...
  dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: "transparent", borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,255,255,0.35)" },
  dotActive: { backgroundColor: theme.colors.primary600, borderColor: theme.colors.primary600 },

  addBtnGrad: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 999 },
  addBtnText: { color: "#06160D", fontWeight: "900" },

  progressWrap: { borderRadius: 16, backgroundColor: "#0E1216", padding: 14, borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,255,255,0.06)" },
  ...
  /* Existing chip styles kept (not used in Progress now but left intact) */
  ...
  rowLabel: { color: "#FFFFFF", fontSize: 16, fontFamily: FONT.displayMed, flex: 1 },
  rowValue: { color: "#FFFFFF", fontSize: 16, fontFamily: FONT.uiBold },

  bigCard: { backgroundColor: "#0E1216", borderRadius: 18, borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,255,255,0.06)", overflow: "hidden" },
  heroImageTop: { width: "100%", height: 188 },
  bigContent: { paddingHorizontal: 14, paddingTop: 10, paddingBottom: 14 },
  bigTitle: { color: "#FFFFFF", fontSize: 18, letterSpacing: 0.2, fontWeight: Platform.select({ ios: "800", android: "700" }) as any },
  bigSub: { color: theme.colors.textLo, fontWeight: "600", marginTop: 4 },
