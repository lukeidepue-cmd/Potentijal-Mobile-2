...
import { listFollowing, searchFollowing, follow, unfollow, type ProfileWithFollowStatus } from "../../../lib/api/social";
...
export default function Following() {
  ...
  // Search following when query changes (debounced)
  useEffect(() => {
    if (!user || !profileId) return;

    const timeoutId = setTimeout(() => {
      if (searchQuery.trim()) {
        searchFollowingDebounced(searchQuery);
      } else {
        loadFollowing();
      }
    }, 300);
  ...
  const loadFollowing = async () => {
    if (!user || !profileId) return;
    setLoading(true);
    const { data, error } = await listFollowing(profileId);
    if (data) {
      setProfiles(data);
  ...
  const searchFollowingDebounced = async (query: string) => {
    if (!user || !profileId) return;
    setLoading(true);
    const { data, error } = await searchFollowing(profileId, query);
    if (data) {
      setProfiles(data);
      const map = new Map<string, boolean>();
      data.forEach((p) => map.set(p.id, p.is_following));
