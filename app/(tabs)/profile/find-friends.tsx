// app/(tabs)/profile/find-friends.tsx
...
import { searchProfiles, listRecommendedProfiles, follow, unfollow, type ProfileWithFollowStatus } from "../../../lib/api/social";
...
export default function FindFriends() {
  ...
  // Load recommended profiles on mount
  useEffect(() => {
    if (!user) return;
    loadRecommended();
  }, [user]);

  // Search profiles when query changes (debounced)
  useEffect(() => {
    if (!user) return;

    const timeoutId = setTimeout(() => {
      if (searchQuery.trim()) {
        searchProfilesDebounced(searchQuery);
      } else {
        loadRecommended();
      }
    }, 300);
  ...
  const loadRecommended = async () => {
    if (!user) return;
    setLoading(true);
    const { data, error } = await listRecommendedProfiles();
    if (data) {
      setProfiles(data);
  ...
  const searchProfilesDebounced = async (query: string) => {
    if (!user) return;
    setLoading(true);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.bg0, paddingTop: insets.top + 16 }}>
      ...
      >
        {/* Title */}
        <Text style={styles.title}>Find Friends and Creators</Text>
            ...
            {profiles.map((profile) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    // Navigate to profile (will show creator workouts if creator)
